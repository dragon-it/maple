...
export const ItemSymbol = ({ symbolData }) => {
  ...
  const arcaneSymbols = symbolList.filter((item) =>
    item.symbol_name.includes("아케인")
  );
  const authenticSymbols = symbolList.filter((item) =>
    item.symbol_name.includes("어센틱")
  );
  const totalArcaneForce = arcaneSymbols.reduce(
    (acc, item) => acc + parseInt(item.symbol_force, 10),
    0
  );
  const totalAuthenticForce = authenticSymbols.reduce(
    (acc, item) => acc + parseInt(item.symbol_force, 10),
    0
  );
  ...
  arcaneSymbols.forEach((item) => {
    ...
    stats.forEach((stat) => {
      ...
      if (statValue > 0) {
        // 해당 스탯의 값을 객체에 저장
        arcStatsObject[stat.name] =
          (arcStatsObject[stat.name] || 0) + statValue;
      }
    });
  });
  ...
  authenticSymbols.forEach((item) => {
    ...
    stats.forEach((stat) => {
      ...
      if (statValue > 0) {
        // 해당 스탯의 값을 객체에 저장
        autStatsObject[stat.name] =
          (autStatsObject[stat.name] || 0) + statValue;
      }
    });
  });
        ...
        <SymbolWrap>
          {arcaneSymbols.map((item, index) => (
            <Items key={index}>
              {/* '아케인' 단어 제거 */}
              <SymbolName>
                {item.symbol_name.replace("아케인심볼 :", "")}
              </SymbolName>
              <SymbolIcons>
